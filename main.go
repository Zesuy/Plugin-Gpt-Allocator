...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if method == nil {
		writeResponse(response, marshalError("invalid_method", "method is required"))
		return 1
	}
	...
	result, err := application().Handle(C.GoString(method), requestBytes)
	if err != nil {
		writeResponse(response, marshalError("plugin_error", err.Error()))
		return 1
	}
	raw, err := json.Marshal(envelope{OK: true, Result: result})
	if err != nil {
		writeResponse(response, marshalError("encode_error", err.Error()))
		return 1
	}
...
func application() *allocatorapp.App {
	appOnce.Do(func() {
		path := os.Getenv("CPA_ROUTE_ALLOCATOR_STATE_PATH")
		if path == "" {
			path = filepath.Join("plugins", "data", allocatorapp.PluginName, "state.json")
		}
		appInst = allocatorapp.New(state.New(path), hostBridge{})
	})
	return appInst
}

func (hostBridge) Call(method string, payload any) (json.RawMessage, error) {
